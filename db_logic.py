...
class DbManager:
    ...
    def get_logged_in_account(self, auth_token=None, uid=None):
        query_from_token = "SELECT is_volunteer, is_super_admin " \
        ...
        if auth_token:
            ...
            if account_info_res:
                is_volunteer, is_super_admin = account_info_res
    ...
    def put_login_session(self, email, pwd):
        """Login (& update token)"""
        query_check_email = "SELECT password, is_volunteer, is_super_admin FROM user_table WHERE email=?"
        ...
        if account_info_result:
            saved_password, is_volunteer, is_super_admin = account_info_result
